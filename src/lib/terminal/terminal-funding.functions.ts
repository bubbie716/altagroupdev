...
export const fetchTerminalFundingEligibility = createServerFn({ method: "GET" }).handler(
  async () => {
    const { isUiLabMode } = await import("@/lib/auth/ui-lab");
    if (isUiLabMode()) {
      const { getUiLabTerminalFundingEligibility } = await import(
...
export const submitTerminalFundingTransferFn = createServerFn({ method: "POST" })
  .inputValidator((input: SubmitTerminalFundingTransferInput) => input)
  .handler(async ({ data }) => {
    const { isUiLabMode } = await import("@/lib/auth/ui-lab");
    if (isUiLabMode()) {
      ...
      return mockUiLabTerminalFundingSubmission(data);
    }
    ...
    return submitTerminalFundingTransfer(user, data);
  });

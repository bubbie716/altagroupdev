...
describe("discord guild role helpers", () => {
  it("resolves configured role ids from env", () => {
    ...
    const originalBank = process.env.DISCORD_BANK_CLIENT_ROLE_ID;
    ...
    try {
      ...
      if (originalClient === undefined) delete process.env.DISCORD_CLIENT_ROLE_ID;
      else process.env.DISCORD_CLIENT_ROLE_ID = originalClient;
      if (originalBank === undefined) delete process.env.DISCORD_BANK_CLIENT_ROLE_ID;
      else process.env.DISCORD_BANK_CLIENT_ROLE_ID = originalBank;
    }
  });

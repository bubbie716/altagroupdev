  ...
  return (
    <section className="rounded-xl border border-gold/30 bg-gold/5 p-5">
      <h3 className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
        Alta Private — relationship intelligence
      </h3>
      <dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 text-[14px]">
        <div>
          <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Eligible</dt>
          <dd className="mt-1 font-medium">{panel.privateBankingEligible ? "Yes" : "No"}</dd>
        </div>
        <div>
          <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Private client</dt>
          <dd className="mt-1 font-medium">{panel.privateBankingClient ? "Yes" : "No"}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-1">{RELATIONSHIP_TIER_LABELS[panel.relationshipTier]}</dd>
        </div>
        <div>
      ...
      {invite ? (
        <p className="mt-4 text-[13px] text-muted-foreground">
          Recommended action: {invite.title} ({invite.confidenceScore}% confidence)
        </p>
      ) : null}
      ...
      <p className="mt-3 text-[12px] text-muted-foreground">
        Mark or remove Private Client via Access tags below — requires confirmation and audit log.
      </p>
    </section>
  );
}

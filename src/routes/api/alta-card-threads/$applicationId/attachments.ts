...
import { uploadAltaCardThreadAttachment } from "@/server/alta-card-application-thread-upload.service";

export const Route = createFileRoute("/api/alta-card-threads/$applicationId/attachments")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        try {
          ...
          const attachment = await uploadAltaCardThreadAttachment(user.id, params.applicationId, file);

...
import type { DiscordMessageDraft } from "@/lib/discord/embed-types";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (!body.draft) {
            return Response.json({ ok: false, message: "Missing message draft payload." }, { status: 400 });
          }
          ...
          if (message.startsWith("DISCORD_API_ERROR:")) {
            return Response.json(
              { ok: false, message: "Discord API rejected the message.", detail: message },
              { status: 502 },
            );
          }
          return Response.json({ ok: false, message: "Unable to process message request." }, { status: 500 });
        }
      },
    },
  },

...
function SupportCenterPage() {
  ...
  const columns =
    communities.length >= 3 ? "sm:grid-cols-3" : communities.length === 2 ? "sm:grid-cols-2" : "sm:grid-cols-1";
  ...
  return (
    <div className="flex min-h-full w-full flex-1 flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-[1400px] flex-1 px-6 py-24">
        ...
        <ul className={`mt-10 grid gap-px overflow-hidden rounded-xl border border-border bg-border ${columns}`}>
          {communities.map((community, index) => {
            ...
            return (
              <li
                ...
                className={
                  isPrimary
                    ? "relative flex flex-col bg-surface-1 p-6 before:absolute before:inset-y-0 before:left-0 before:w-[2px] before:bg-gold sm:p-7"
